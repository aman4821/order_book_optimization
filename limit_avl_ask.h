/*
This header file implements the limits data structure for the ask side.
...
constitutes of a set of orders for that particular price modelled as a Min Heap.
    ...
    node* insertNode(int orderID, int orderLimitPrice, int orderShares, int orderEntryTime, int isOrderAsk, node* root)
    {
       ...
       if(root == NULL)
        ...
        else if(orderLimitPrice > root->limitPrice)
        {
            ...
            root->currentLimitOrders.push_back(currOrder);
            std::make_heap(root->currentLimitOrders.begin(), root->currentLimitOrders.end(), min_heap_limit_price_comparator());
        }
